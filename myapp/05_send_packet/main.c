...
static __attribute((noreturn)) void lcore_main(struct rte_mempool* mbuf_pool)
{
    ...
    struct rte_mbuf* send_buffer[1];
    send_buffer[0] = rte_pktmbuf_alloc(mbuf_pool);
    if (send_buffer[0] == NULL) {
    ...
    memcpy(rte_pktmbuf_mtod(send_buffer[0], void*), raw, sizeof raw);
    send_buffer[0]->nb_segs = 1;
    send_buffer[0]->next = NULL;
    send_buffer[0]->pkt_len  = (uint16_t)sizeof(raw);
    send_buffer[0]->data_len = (uint16_t)sizeof(raw);

    rte_pktmbuf_dump(stdout, send_buffer[0], sizeof(struct rte_mbuf));
    ...
    rte_pktmbuf_free(send_buffer[0]);
}
